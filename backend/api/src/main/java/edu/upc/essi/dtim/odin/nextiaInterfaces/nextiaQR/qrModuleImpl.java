...
public class qrModuleImpl implements qrModuleInterface {
    ...
    public QueryResult makeQuery(IntegratedGraphJenaImpl integratedGraph, List<Dataset> integratedDatasets, QueryDataSelection body) {
        ...
        String filePath = "api/src/main/resources/";
        File directory = new File(filePath);
        if (!directory.isDirectory()) { // For Docker
            filePath = "/backend/api/src/main/resources/";
        }

        dl.execute("DROP TABLE IF EXISTS for_countries");
        dl.execute("CREATE TABLE IF NOT EXISTS for_countries AS SELECT * FROM read_csv_auto('" + filePath + "anomalies_binary.csv')");
        dl.uploadToTemporalExploitationZone("SELECT * FROM for_countries", UUID);
        // End of the mock

...
public class mapgenModuleImpl implements mapgenModuleInterface {
    ...
    public MapgenResult generateMappings(String mappingType, Graph graphI) {
        MapgenODIN mapgenODINInterface;
        try {
            mapgenODINInterface = MapgenFactory.getInstance(mappingType, graphI);
            return mapgenODINInterface.generateMappings(mappingType, graphI);
        } catch (Exception e) {
            e.printStackTrace();
